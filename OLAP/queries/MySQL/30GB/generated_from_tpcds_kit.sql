...
with ssr as
 ...
 where date_sk = d_date_sk
       and d_date between cast('2000-08-19' as date) 
                  and date_add(cast('2000-08-19' as date), interval 14 day)
...
select case when (select count(*) 
                  from store_sales 
                  where ss_quantity between 1 and 20) > 92190
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 764847
            then (select avg(ss_ext_discount_amt)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 1213327
            then (select avg(ss_ext_discount_amt)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 1232224
            then (select avg(ss_ext_discount_amt)
                  from store_sales
...
select  
 ...
 where
  ...
  ca_county in ('New Haven County','Claiborne County','Portage County','Cotton County','Jennings County') and
...
where 
	ws_item_sk = i_item_sk 
  	and i_category in ('Men', 'Shoes', 'Children')
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('1998-05-05' as date) 
				and date_add(cast('1998-05-05' as date), interval 30 day)
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('IN', 'GA', 'KS')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('SD', 'PA', 'FL')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('TX', 'KY', 'MI')
...
where
    d_date between '2001-5-01' and 
           date_add(cast('2001-5-01' as date), interval 60 day)
and cs1.cs_ship_date_sk = d_date_sk
and cs1.cs_ship_addr_sk = ca_address_sk
and ca_state = 'WI'
and cs1.cs_call_center_sk = cc_call_center_sk
and cc_county in ('Williamson County','Ziebach County','Ziebach County','Williamson County',
                  'Ziebach County'
)
and exists (select *
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (10,7,6,11,8,4) and
       d_year = 1999 and
       ca_state in ('IL','IA','NE'
                   ,'WI','CA','TX','GA')
...
select  i_item_id
 ...
 where cs_item_sk = i_item_sk 
   ...
   and cs_sold_date_sk = d_date_sk
 and d_date between cast('2000-03-18' as date) 
 				and date_add(cast('2000-03-18' as date), interval 30 day)
 group by i_item_id
         ,i_item_desc 
         ,i_category
         ,i_class
...
select  *
 from(select w_warehouse_name
            ,i_item_id
            ,sum(case when (cast(d_date as date) < cast('2002-06-19' as date))
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between date_sub(cast('2002-06-19' as date), interval 30 day)
                    and date_add(cast('2002-06-19' as date), interval 30 day)
...
and d_date between '2001-01-28' and 
        date_add(cast('2001-01-28' as date), interval 90 day)
...
select  i_item_id
 ...
 and d_date between cast('2000-07-10' as date) and date_add(cast('2000-07-10' as date), interval 60 day)
...
select  
   w_state
  ,i_item_id
  ,sum(case when (cast(d_date as date) < cast('1999-03-15' as date)) 
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between date_sub(cast('1999-03-15' as date), interval 30 day)
                and date_add(cast('1999-03-15' as date), interval 30 day)
 group by
    w_state,i_item_id
...
select  c_last_name
 ...
 from
   (select ss_ticket_number
    ...
    and date_dim.d_year in (1998,1998+1,1998+2) 
    and store.s_city in ('Fairview','Midway','Five Points','Oak Grove','Fairview') 
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('firebrick','seashell','peach'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 1998
 and     d_moy                   = 2
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('firebrick','black','bisque','papaya','coral','aquamarine') and
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 2002
 	and t_time between 5525 and 5525+28800 
...
with ss as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1998-08-22' as date) 
                  and date_add(cast('1998-08-22' as date), interval 30 day) 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('2002-08-27' as date) 
                  and date_add(cast('2002-08-27' as date), interval 30 day)
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
group by web_site_id)
 ...
 from 
 (select 'store channel' as channel
        , concat('store', ifnull(store_id, '')) as id
        , sales
        , returns
        , profit
 from   ssr
 union all
 select 'catalog channel' as channel
        , concat('catalog_page', ifnull(catalog_page_id, '')) as id
        , sales
        , returns
        , profit
 from  csr
...
select  i_item_id
 ...
 and inv_item_sk = i_item_sk
 and d_date_sk=inv_date_sk
 and d_date between cast('2000-01-31' as date) and date_add(cast('2000-01-31' as date), interval 60 day)
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1998-02-21','1998-08-29','1998-11-09')))
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Union Hill'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  26516
   and ib_upper_bound   <=  26516 + 50000
...
from ((select distinct c_last_name, c_first_name, d_date
       ...
       where store_sales.ss_sold_date_sk = date_dim.d_date_sk
         and store_sales.ss_customer_sk = customer.c_customer_sk
         and d_month_seq between 1217 and 1217+11 AND NOT EXISTS
      (select distinct c_last_name, c_first_name, d_date
       from catalog_sales, date_dim, customer
       where catalog_sales.cs_sold_date_sk = date_dim.d_date_sk
         and catalog_sales.cs_bill_customer_sk = customer.c_customer_sk
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (1998) and
        ((i_category in ('Women','Children','Shoes') and
          i_class in ('dresses','newborn','athletic')
         )
      or (i_category in ('Electronics','Books','Home') and
          i_class in ('dvd/vcr players','mystery','kids') 
        ))
...
and d_date between '2001-02-10' and 
        date_add(cast('2001-02-10' as date), interval 90 day)
...
where 
	ss_item_sk = i_item_sk 
  	and i_category in ('Home', 'Women', 'Sports')
  	and ss_sold_date_sk = d_date_sk
